...
// A switch for appending uniform error scalings to find DeltaChi2 and n3sig cuts
  ...
  filekeywords()
  {
    ...
    //basedir= new char[1000]; //do not alloc, it will be handled by std::getenv
    starsdir= new char[1000];
  ...
  ~filekeywords()
  {
    delete[] pathdir;
    delete[] pathfile;
    //delete[] basedir; //do not delete, it holds pointer to return of getenv
    delete[] starsdir;
    delete[] obsdir;
    delete[] obslist;
...
struct event{
  ...
  vector<string> obsgroupoutputheader; //The names of columns in the obsgroup output
  ...
  vector<vector<double> > jdtimes; //an array of the JD times for each observatory
  vector<double> alt;
  vector<int> obsidx;
  vector<int> nstack;
  vector<double> texp;
