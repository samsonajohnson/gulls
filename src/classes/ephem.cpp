...
void orbitalElements::viewfrom(double t, double ra, double dec, vector<double>* view, int eqecl)
{
  ...
  settime(t);
  compute();
  viewfrom(ra, dec, view, eqecl);
}

void orbitalElements::viewfrom(double ra, double dec, vector<double>* view, int eqecl)
{
  ...
  //eqecl is a switch for choosing whether to work in equatorial(default) or ecliptic(1) coordinates

  //Start off working in the cartesian celestial frame where x points to the vernal equinox, z to the NEP, y in the direction of increasing RA

  double cr, cd, sr, sd; //cosine and sin of the eq or ecl coordinates
  double lam, bet;
  coords c;

  if(eqecl==1)
	{
	  c.ad2ecl(ra,dec,&lam,&bet);
	  cr = cos(lam); cd = cos(bet);
	  sr = sin(lam); sd = sin(bet);
	}
  else
	{
	  cr = cos(ra); cd = cos(dec);
	  sr = sin(ra); sd = sin(dec);
	}
  ...
  //In the sky frame:
  //the east vector on the sky will be perpendicular to both the normal and NCP/NEP  vectors
  ...
  if(eqecl==1)
	{
	  obj_NEn[0] = Nhat[0]*xecl + Nhat[1]*yecl + Nhat[2]*zecl;
	  obj_NEn[1] = Ehat[0]*xecl + Ehat[1]*yecl + Ehat[2]*zecl;
	  obj_NEn[2] = nhat[0]*xecl + nhat[1]*yecl + nhat[2]*zecl;
	}
  else
	{
...
void orbitalElements::velocity(double t, double ra, double dec, vector<double>* v, int eqecl, double tstep)
{
  ...
  viewfrom(t-tstep,ra,dec,&x1,eqecl);
  viewfrom(t+tstep,ra,dec,&x2,eqecl);
...
void orbitalElements::acceleration(double t, double ra, double dec, vector<double>* acc, int eqecl, double tstep)
{
  ...
  viewfrom(t-tstep,ra,dec,&x1,eqecl);
  viewfrom(t,ra,dec,&x2,eqecl);
  viewfrom(t+tstep,ra,dec,&x3,eqecl);
...
void orbitalElements::eclpropermotion(double t, double ra, double dec, double dist, vector<double>* pm, double tstep)
{
  //dist in kpc
  ...
  c.ad2ecl(ra,dec,&lambda,&beta);
  
  velocity(t, ra, dec, &v, tstep, 1);

  pm->resize(2);
  (*pm)[0] = scale*v[0]*cos(beta);
  (*pm)[1] = scale*v[1];
  (*pm)[2] = v[2];
...
void orbitalElements::eq2ecl(vector<double> eq, vector<double>* ecl)
{
  coords c;

  double lambda, beta;

  c.ad2ecl(eq[0],eq[1],&lambda,&beta);
  (*ecl)[0] = lambda;
  (*ecl)[1] = beta;
}
 
void orbitalElements::ecl2eq(vector<double> ecl, vector<double>* eq)
{
  coords c;
  
  double a,d;

  c.ecl2ad(ecl[0],ecl[1],&a,&d);
  (*eq)[0] = a;
  (*eq)[1] = d;  
}
