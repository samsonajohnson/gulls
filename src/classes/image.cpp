...
int image::load_detector(string filename,vector<string> modifications)
{
  int status=0;
  ...
  //read in keys from the parameter file
  while(!in.eof())
    {
      getline(in,line);
      split(line,data);
  ...
  vector<string> mdata;
  vector<int> modded = vector<int>(keys.size(),0);
  for(int modidx=0; modidx<modifications.size(); modidx++)
    {
      split(modifications[modidx],mdata,string(1,'='));
      for(int i=0;i<int(keys.size());i++)
	{
	  if(!keys[i].compare(mdata[0])) //a match
	    {
	      if(i!=15) //numerical values
		{
		  par[i] = atof(mdata[1].c_str());
		}
	      else //string values
		{
		  psffile = mdata[1];
  ...
  for(int i=0; i<int(keys.size()); i++)
    {
      ...
      if(modded[i]==0)
	{
	  cerr << "not modified" << endl;
	}
      else
	{
	  cerr << "modified to " << par[i] << endl;
	}
    }

  //Warn of the use of default parameters
  for(int i=0;i<int(keys.size());i++)
    {
      if(!set[i])
	{
...
void image::minimal_image(double ell, double b)
{
  if(!psf.init)
    {
...
void image::setup_fast_photometry(int _xsub, int _ysub, int x, int y, double magnitude, double texp_, double nstack_, bool backg, aperture* ap)
{
  ...
  vector<double> truethere, truenotthere;
  ...
  cout << "idealDetCounts bgcounts mag " << idealDetCounts << " " << bgcounts << " " << magnitude << endl;

  //Generate an ideal exposrue without the source in the image
  reset_detector();
  ideal_expose(texp_,nstack_);
  //write_truefits(string("sfp_start_nosource.fits"), true);
	  ...
	  if( ap->aper[jap+i]>0 && ii>=0 && ii<Xpix && jj>=0 && jj<Ypix)
	    {
	      ...
	      truenotthere.push_back(timage[imidx]);
	    }
	}
    }
  ...
  //write_truefits(string("sfp_withsource.fits"), true);
	  ...
	  if( ap->aper[jap+i]>0 && ii>=0 && ii<Xpix && jj>=0 && jj<Ypix)
	    {
	      ...
	      truethere.push_back(timage[imidx]);
	    }
	}
    }
  ...
  for(int i=0;i<int(there.size());i++)
    {      
      ...
      fast_src += (nstack_*texp_*(truethere[i]-truenotthere[i]));
      fast_blend += (nstack_*texp_*truethere[i]-nstack_*bgcounts);
      fast_notthere += (nstack_*texp_*truenotthere[i]-nstack_*bgcounts);
      fast_total += (nstack_*texp_*truethere[i]+nstack_*(idealDetCounts));
    }
 
  cout << "setup_fast_photometry " << fast_satlimit << " " << fast_src << " " << fast_blend << " " << fast_total << " " << fast_notthere << endl;
...
{
  //Calculate the size of field over which to draw stars
  //first work in pixels
...
{
  //Calculate the size of field over which to draw stars if the field can be rolled
  //first work in pixels
