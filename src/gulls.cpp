...
int main(int argc, char *argv[]){                   /* BEGIN MAIN */
  ...
  starfield = new vector<vector<vector<double> > >();
  starfieldData = new vector<double>();
  ...
  if(getValidFields(&Paramfile, &Sources, &Lenses, starfieldData)==0)
    {
	  ...
	  cerr << "   Often this error is caused by an incorrect value in the NFILTERS value in the parameter file" << endl;
      exit(1);
    }

  cout <<  Paramfile.validFields.size() << " fields read: " << Sources.end << " sources, " << Lenses.end << " lenses" << endl;
