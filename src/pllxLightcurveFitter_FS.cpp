...
int lightcurveFitter_FS(struct filekeywords* Paramfile, struct obsfilekeywords World[], struct event *Event, int enablePllx)
{
  ...
  //Event->FSPL[obsgroup].pllx.resize(Paramfile->numobservatories);
  /*
  for(grpidx=0;grpidx<int(Event->obsgroups[obsgroup].size());grpidx++)
    {
      obsidx=Event->obsgroups[obsgroup][grpidx];

      //cout << "FS Setting up parallax for " << obsgroup << " "<< grpidx << " " << obsidx << endl;

      //Event->PSPL[obsgroup].pllx.push_back(Event->pllx[obsidx]);
       
      Event->FSPL[obsgroup].pllx[obsidx].set_lb(Event->l, Event->b);
      Event->FSPL[obsgroup].pllx[obsidx].setup_reference_frame(Paramfile->simulation_zerotime+tref,&World[0].orbit);
      Event->FSPL[obsgroup].pllx[obsidx].set_orbit(&World[obsidx].orbit);
      Event->FSPL[obsgroup].pllx[obsidx].provide_observables_NE(Event->piEN, Event->piEE,Event->tE_r);
       
      //Do this for a dummy epoch now, but redo this at the end of 
      //timeSequencer
      Event->FSPL[obsgroup].pllx[obsidx].load_epochs(&World[obsidx].jd);
      Event->FSPL[obsgroup].pllx[obsidx].compute_NEshifts();
