...
int lightcurveFitter(struct filekeywords* Paramfile, struct obsfilekeywords World[], struct event *Event, int enablePllx)
{
  ...
  //Event->PSPL[obsgroup].pllx.resize(Paramfile->numobservatories);
  /*
  for(grpidx=0;grpidx<int(Event->obsgroups[obsgroup].size());grpidx++)
    {
      ...
      //cout << "Setting up parallax for " << obsgroup << " " << obsidx << endl;
      
      Event->PSPL[obsgroup].chisqvec[obsidx]=0;

      Event->PSPL[obsgroup].pllx[obsidx].set_lb(Event->l, Event->b);
      Event->PSPL[obsgroup].pllx[obsidx].setup_reference_frame(Paramfile->simulation_zerotime+tref,&World[0].orbit);
      Event->PSPL[obsgroup].pllx[obsidx].set_orbit(&World[obsidx].orbit);
      Event->PSPL[obsgroup].pllx[obsidx].provide_observables_NE(Event->piEN, Event->piEE,Event->tE_r);
      
      //Do this for a dummy epoch now, but redo this at the end of 
      //timeSequencer
      Event->PSPL[obsgroup].pllx[obsidx].load_epochs(&World[obsidx].jd);
      Event->PSPL[obsgroup].pllx[obsidx].compute_NEshifts();
      //Event->pllx[i].initialize();
    }
  */
  ...
  if(Event->flatchi2[obsgroup]<Paramfile->minChiSquared)
    {
  ...
  if(Paramfile->pllxMultiplyer!=0&&enablePllx)
    {
      ...
      gsl_vector_set (ss,3, 0.001);
      gsl_vector_set (ss,4, 0.001);
    }
  else
    {
      minex_func.n = 3;
	  ...
	  for(grpidx=0;grpidx<int(EventL->obsgroups[obsgroup].size());grpidx++)
	    {
	      ...
	      EventL->pllx[obsidx].provide_observables_NE(piEN, piEE, tE);
	      EventL->pllx[obsidx].compute_tushifts();
	    }
	}
    }
		  ...
		  if(pllx)
		    {
		      ...
		      //tt += EventL->pllx[obsidx].tshift(EventL->jdepoch[idx]);
		      //uu += EventL->pllx[obsidx].ushift(EventL->jdepoch[idx]);
		      tt += EventL->pllx[obsidx].tshift[shiftedidx];
		      uu += EventL->pllx[obsidx].ushift[shiftedidx];
